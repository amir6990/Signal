...
basPath = fso.BuildPath(here, "vba\dist\SignalRefresh.bas")
Dim basTime
basTime = fso.BuildPath(here, "vba\dist\TimeCycles.bas")
Dim basMacro
basMacro = fso.BuildPath(here, "vba\dist\MacroTime.bas")
Dim basStats
basStats = fso.BuildPath(here, "vba\dist\MacroStats.bas")
Dim basOpt
basOpt = fso.BuildPath(here, "vba\dist\OptionsRefresh.bas")

If Not fso.FileExists(basPath) Then
    MsgBox "فایل ماژول پیدا نشد:" & vbCrLf & basPath & vbCrLf & vbCrLf & _
           "پوشه vba\dist باید کنار همین اسکریپت باشد." & vbCrLf & _
           "اگر نیست، یک بار اجرا کنید:  python scripts/vba_pack.py", _
           16, "نصب ناموفق"
    WScript.Quit 1
End If
