...
# ------------------------------------------------- ۷) چشم‌انداز احتمالاتی
FC_ROWS = [
    ("شاخص کل بورس", "tedpix", 3),
    ("دلار آزاد", "usd", 5),
    ("اونس طلا", "ons", 8),
    ("سکه تمام", "coin", 9),
]
...
FC_COLS = [
    ("دارایی", 18, None),
    ("رژیم فعلی", 13, None),
    ("احتمال رژیم پرتنش", 15, F_PCT),
    ("بازده سالانه رژیم آرام", 16, F_PCT),
    ("بازده سالانه رژیم پرتنش", 16, F_PCT),
    ("نوسان آرام", 12, F_PCT),
    ("نوسان پرتنش", 12, F_PCT),
    ("ماندگاری آرام (روز)", 14, F_NUM1),
    ("ماندگاری پرتنش (روز)", 14, F_NUM1),
    ("احتمال ورود به تنش (۳ ماه)", 17, F_PCT),
    ("مدل: بازده ۳ماهه < ‎−۱۰٪", 17, F_PCT),
    ("تجربی: همان رویداد", 15, F_PCT),
    ("n مؤثر", 10, "0"),
    ("هشدار", 40, None),
]
...
def build_forecast(wb):
    """چشم‌انداز سه‌ماهه هر متغیر کلان — نه پیش‌بینی نقطه‌ای.

    این شیت عدد قطعی نمی‌دهد و نباید بدهد. سه برآورد **هم‌جنس** از یک
    رویداد مشخص می‌دهد و کنار هم می‌گذارد:
      ۱. مدل رژیم مارکوف (همیلتون ۱۹۸۹) با EM
      ۲. شبیه‌سازی مونت‌کارلو از همان مدل
      ۳. نرخ پایه تجربی، با تصحیح همپوشانی پنجره‌ها

    اگر این سه با هم اختلاف زیاد داشتند، خودِ اختلاف اطلاعات است: یعنی
    نتیجه به فرض‌های مدل حساس است و نباید به آن تکیه کرد.
    """
    ws = _sheet(wb, "Forecast",
                "چشم‌انداز احتمالاتی سه‌ماهه",
                "احتمال، نه پیش‌بینی. «۳۰٪ احتمال افت بیش از ۱۰٪» یعنی اگر "
                "صد بار در چنین وضعیتی باشیم، حدود سی بار چنین می‌شود — نه "
                "اینکه بازار سقوط می‌کند.", len(FC_COLS))
    widths(ws, [c[1] for c in FC_COLS])
    hdr(ws, 3, [c[0] for c in FC_COLS],
        ["-", "همیلتون", "هموارشده", "EM", "EM", "EM", "EM",
         "۱÷(۱−p)", "۱÷(۱−p)", "زنجیره مارکوف", "مونت‌کارلو",
         "نرخ پایه", "n÷افق", "-"])
    for i, (lbl, _k, _c) in enumerate(FC_ROWS):
        r = FIRST + i
        ws.cell(row=r, column=1, value=lbl)
        for j, (_l, _w, fmt) in enumerate(FC_COLS, start=1):
            c = ws.cell(row=r, column=j)
            c.font = Font(name=FONT, size=8, bold=(j == 1))
            c.alignment = Alignment(horizontal="center", vertical="center",
                                    wrap_text=(j == len(FC_COLS)))
            c.border = BORDER
            if fmt:
                c.number_format = fmt
        ws.row_dimensions[r].height = 26

    last = FIRST + len(FC_ROWS) - 1
    for col in ("C", "J", "K"):
        ws.conditional_formatting.add("%s%d:%s%d" % (col, FIRST, col, last),
                                      ColorScaleRule(
    ...
    r2 = last + 2
    for txt in [
        "**دو ستون آخرِ احتمال، عمداً یک رویداد را می‌سنجند:** «بازده سه‌ماهه "
        "کمتر از ‎−۱۰٪». یکی از مدل می‌آید و یکی از تاریخ. اگر ناهم‌جنس بودند "
        "(مثلاً یکی افت درون‌دوره و دیگری بازده پایان‌دوره)، مقایسه‌شان بی‌معنا "
        "می‌شد — و این دقیقاً خطایی است که راحت رخ می‌دهد.",
        "**«رژیم پرتنش» یعنی چه:** مدل دو حالت پیدا می‌کند و حالتی که "
        "میانگین بازده کمترش است «پرتنش» نام می‌گیرد. این برچسب از داده "
        "درمی‌آید، نه از قضاوت من.",
        "⚠️ **مسئله همپوشانی پنجره‌ها.** نرخ پایه سه‌ماهه از پنجره‌های "
        "متداخل حساب می‌شود، پس تعداد مشاهده مستقل بسیار کمتر از تعداد "
        "ردیف است. ستون «n مؤثر» عدد واقعی را نشان می‌دهد (تعداد ÷ افق). "
        "اگر n مؤثر زیر ۱۰ باشد، نرخ پایه یک حکایت است نه آمار.",
        "⚠️ هر سه ستون احتمال، **مشروط بر ادامه رفتار گذشته**اند. تغییر "
        "رژیم سیاستی (جهش ارزی، تحریم جدید، تغییر قواعد بازار) دقیقاً "
        "چیزی است که مدل نمی‌تواند ببیند، چون در داده گذشته نیست.",
    ]:
        note(ws, "A%d" % r2, txt, size=9, color="404040")
        ws.merge_cells(start_row=r2, start_column=1, end_row=r2,
                       end_column=len(FC_COLS))
